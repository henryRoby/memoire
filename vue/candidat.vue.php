<?php
    ...
    $errreur = "";
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['cv']) == "POST" && isset($_FILES['lettre']) == "POST") 
    {

        if (empty($_POST["email"]) AND empty($_FILES["cv"]["name"])AND empty($_FILES["lettre"]["name"]))
        {
            $errreur = "Tous les champs sont obligatoire";
        }
        else
        { 
              ...
              {
                $errreur = "nom de fichier tres long";
              }
              else
              {
                    ...
                    if($bool_email)
                        {
                          $errreur = "vous avez dejà postulé";
                        }
                    else
                        {
                          $errors= array();
                          ...
                          $file_type1=$_FILES['lettre']['type'] ;
                          
                          $teste_ext = explode('.',$_FILES['cv']['name']);
                          $teste_ext1 = explode('.',$_FILES['lettre']['name']);

                          $file_ext=strtolower(end($teste_ext));
                          $file_ext1=strtolower(end($teste_ext1));
                          
                          $extensions= array("pdf");
                          
                          if(in_array($file_ext,$extensions)=== false || in_array($file_ext1,$extensions)=== false ){

                            $errreur = "Entrez une éxtension pdf";
                          ...
                          else 
                          {
                            ...
                            header("Location:confirmation.vue.php ");
                          }
...
<meta name="theme-color" content="#7952b3">
  ...
  <div class="myform">
    ...
    <div class="form-group">
        ...
        <a id="soumettre" class="btn btn-danger" href="accueil.php">Annuler</a> </br>
        <?php
        echo $errreur;  
      ?>
        </center>
